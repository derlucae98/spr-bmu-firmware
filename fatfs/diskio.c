...
int wait_ready (void)   /* 1:OK, 0:Timeout */
{
    ...
    if (spi_mutex_take(SD_SPI, portMAX_DELAY)) {
        for (; tmr; tmr--) {  /* Wait for ready in timeout of 500ms */
            rcvr_mmc(&d, 1);
            if (d == 0xFF) break;
...
static
int select_card (void)   /* 1:OK, 0:Timeout */
{
    BYTE d;
...
DWORD get_fattime() {
    ...
    rtc_date_time_t rtcDateTime = rtc_get_date_time();
